...
- コード内のコメントは [コードコメント（`App/`）](#コードコメントapp) に従う。言語は周囲の
  既存コメントに合わせる。
...
`App/` 配下の Swift コメントはデフォルトで書かない。書くのは設計判断の根拠（Why / why not）だけに
限る。長期的な方針は [`Docs/adr/`](Docs/adr/) に置き、コメントに溜めない。

してはいけないこと:

- 型・プロパティ・メソッド名から読み取れる説明を書く
- `///` で API ドキュメントのように「何をするか」を列挙する
- 処理の what / how をステップで書く
- `// MARK:` でセクションを区切る（構造がコードから読み取れるとき）
- TF-NNNN や Issue # 番号だけをラベルとして残す
- 変更履歴・実装メモ・「ここを直した」旨を残す
- コードと重複する説明文を増やす
- 根拠のないコメントを「念のため」追加する
- 文の途中で終わる未完のコメントを残す
- テスト名や型名で足りることを `///` で繰り返す

根拠コメントを書いてよいのは、次のような非自明な判断に限る:

- なぜその実装か（代替を選ばなかった理由、制約、互換性）
- なぜ別のやり方をしないか（誤読・デッドロック・データ汚染・表示の壊れ方の防止）
