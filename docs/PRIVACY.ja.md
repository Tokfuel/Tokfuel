...
Tokfuel は個人データを収集せず、利用データをどこにも送信しません。読み取ったデータ（Claude Code の transcript や Skill 一覧）はすべて Mac の中にとどまります。アカウント、トラッキング、アナリティクス、サードパーティ SDK はいずれもありません。為替レートの取得や、Cursor を利用している場合は Cursor 自身の価格表・利用量の取得など、限定的で開示済みの通信のみ行います（詳細は下記）。利用データそのものを送信することはありません。
...
- **Codex CLI のセッションログ**（`~/.codex/sessions/` 以下、存在する場合）：セッション数とトークン数の集計のため端末上でのみ読み取ります。この集計はまだアプリの UI（コスト・グラフ・メニューバーいずれにも）には反映されていませんが、ファイル自体は読み取っているためここに明記します。
