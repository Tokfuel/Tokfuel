...
Tokfuel は個人データを収集せず、利用データをどこにも送信しません。読み取ったデータ（Claude Code の transcript や Skill 一覧）はすべて Mac の中にとどまります。アカウント、トラッキング、アナリティクス、サードパーティ SDK はいずれもありません。為替レートの取得、GitHub Releases への新バージョンの確認、Cursor を利用している場合は Cursor 自身の価格表・利用量の取得など、限定的で開示済みの通信のみ行います（詳細は下記）。利用データそのものを送信することはありません。
...
Tokfuel が通信するのは以下のケースのみで、Claude Code の transcript や Skill 利用状況を送信することはありません。
...
- **アプリの更新確認（自動）**：アプリ内アップデートを提案するため、`UpdateChecker` が起動時と以後 24 時間ごとに、GitHub の公開 Releases API（`api.github.com`）へ Tokfuel の最新リリースを問い合わせます。リクエストに利用データや識別子は含みません。リリースファイル自体のダウンロードは、ポップオーバーのバナーで**アップデート**を押したときにのみ行います。
...
一般の Web リクエストと同様、これらの API の運営者は IP アドレスなどの標準的な接続メタデータを技術的に受け取ります。Cursor が未導入であれば、発生しうるのは更新確認、バナーから明示的に実行したリリースのダウンロード、為替レートのリクエスト（JPY をオプトインした場合のみ）だけです。
