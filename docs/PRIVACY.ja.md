...
Tokfuel は個人データを収集せず、プロンプトやローカルの transcript を送信しません。Claude Code から読み取ったデータはすべて Mac の中にとどまります。アカウント、トラッキング、アナリティクス、サードパーティ SDK はいずれもありません。為替レートの取得、GitHub Releases への新バージョンの確認、Cursor を利用している場合は Cursor 自身の価格表とダッシュボード利用量の取得など、限定的で開示済みの通信のみ行います（詳細は下記）。
