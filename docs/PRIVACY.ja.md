...
一般の Web リクエストと同様、これらの API の運営者は IP アドレスなどの標準的な接続メタデータを技術的に受け取ります。Cursor が未導入であれば、発生しうるのは更新確認、アップデートボタンから明示的に実行したリリースのダウンロード、為替レートのリクエスト（JPY をオプトインした場合のみ）だけです。
