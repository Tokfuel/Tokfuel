...
description: >-
  ...
  the feature (that is the implement-cu skill).
---
...
[`implement-cu`](../implement-cu/SKILL.md), or — when the item is small and its design is settled —
...
3. **retok is vendored unmodified.** Don't propose editing the bundled retok in place (upstream PR
   instead); keep its MIT license and attribution intact. See [CU-0002](../../../roadmaps/CU-0002-native-swift-cost-analysis/CU-0002-native-swift-cost-analysis.md)
...
CU item, its topic and status — and the specific `CU-NNNN-*/` files relevant to the topic. Use
...
CU-00xx — extend it, or is it distinct?").

### 3. Classify each surviving idea — tell the user which and why

- **Overlaps an existing CU item** → don't duplicate. Augment that item's files (both languages).
- **Novel and scoped enough** → draft a new CU item (step 4).
- **Still unformed** → add a bullet under **Unsorted ideas** in both READMEs; promote later.

### 4. Draft a new CU item

Allocate the next ID = highest existing `CU-NNNN` + 1:

```bash
ls -d roadmaps/CU-*/ | sort | tail -1
```

Create `roadmaps/CU-NNNN-<slug>/` with both `CU-NNNN-<slug>.md` and `CU-NNNN-<slug>-ja.md`.
Copy the shape from an existing item (e.g. [CU-0001](../../../roadmaps/CU-0001-budget-alerts/CU-0001-budget-alerts.md)):
the bilingual header link, the `<!-- CU-METADATA -->` block (`Proposal` / `Author` / `Status: Proposal`
...
(`docs(roadmap): add CU-NNNN <slug>`). Open a PR only if the user asks.

## References

- [`roadmaps/README.md`](../../../roadmaps/README.md) — the index and the per-item format.
- [`implement-cu`](../implement-cu/SKILL.md) — the counterpart that ships a numbered item.
