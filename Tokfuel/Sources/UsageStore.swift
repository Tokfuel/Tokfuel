...
final class UsageStore: ObservableObject {
    ...
    /// RetokTime に委譲する（書式の基準はそちら。タイムゾーンは呼び出しごとに解決するので
    /// 常駐アプリ稼働中の変更にも追随する）。actor 状態に触れない純粋関数なので nonisolated —
    /// CursorUsageReader のようにバックグラウンドから同期的に呼びたい場所からも await なしで使える。
