...
struct ReportCache: Sendable {
    ...
    /// キーは retok の実行を左右する入力（期間・言語・走査ディレクトリ）すべてから組む。
    /// どれか 1 つでも欠くと、その設定を切り替えた直後に別条件のレポートを再生してしまう。
    private func fileURL(days: Int, lang: String, projectsPath: String) -> URL {
        let digest = SHA256.hash(data: Data(projectsPath.utf8))
            .prefix(4).map { String(format: "%02x", $0) }.joined()
        return directory.appendingPathComponent("report-\(days)d-\(lang)-\(digest).json")
    }

    func load(days: Int, lang: String, projectsPath: String) -> RetokReport? {
        let url = fileURL(days: days, lang: lang, projectsPath: projectsPath)
    ...
    func save(_ report: RetokReport, days: Int, lang: String, projectsPath: String) {
        guard let data = try? JSONEncoder().encode(report) else { return }
        try? FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        try? data.write(to: fileURL(days: days, lang: lang, projectsPath: projectsPath),
                        options: .atomic)
    }
}
