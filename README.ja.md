...
- 🖱️ **Cursor も**
  ...
  Cursor がインストールされていれば、そのローカルな使用量も同じ合計とグラフに含めます。
  トークンスナップショットが取れないメッセージもあるため、常に実際の使用量を下回る推定値です。
  設定は不要です。価格表は Cursor 公式の公開表から 1 日 1 回更新し、取得できないときは
  同梱の表にフォールバックします。
...
- 🔒 **ローカルファースト**
  ...
  テレメトリなし。通信するのは為替レート取得（オプトイン）と、Cursor 導入時の価格表の
  1 日 1 回の更新だけです。どちらも使用データは送信しません。
