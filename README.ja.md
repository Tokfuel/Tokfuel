...
- 📏 **サーバー真値のリミット表示（オプトイン）** — 設定のトグル（既定 OFF）を有効にすると、公式 `/usage` コマンドと*同じ* 5 時間・週次の使用率をリセットまでの時間・プランバッジ付きで表示します（プラン検出自体は完全ローカル）。Claude Code が保存済みの OAuth トークン（ファイルまたは Keychain）を再利用し、**そのトークンだけを Anthropic だけに**送ります。本アプリ唯一の通信機能で、それ以外は完全オフラインのままです。トークンの更新は行いません（失効時は Claude Code を開いてください）。手法は [CodexBar](https://github.com/steipete/CodexBar)（MIT）の調査に基づきます。
