...
- 📤 **CSV 書き出し**
  ...
  ポップオーバーの ⋯ メニューに「日別」「月別」2 つの書き出し項目があり、表示中の期間を
  CSV に保存します。日別は 1 行 1 日（日付・コスト・出力トークン数）、月別は同じ値を
  暦月単位で合算して 1 行 1 か月にします。どちらも期間合計（プロンプト数・API
  リクエスト数・キャッシュヒット率）とモデル別のコスト内訳を添えます。すべて retok の
  レポートに元からある値で、新しい走査はしません。チームの支出を手元で集計したい
  管理者向けの機能で（各メンバーが書き出したファイルを共有する運用を想定）、
  書き出し自体はどこにも送信しません。
...
- アーキテクチャ決定: [ADR/](ADR/README.ja.md)（[English](ADR/README.md)）
