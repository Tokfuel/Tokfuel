...
- 🛠️ **設定** — 歯車ボタンから設定ウィンドウを開けます: ログイン時起動、メニューバー表示（コスト / プロンプト数 / アイコンのみ）、既定の集計期間、レポート言語、そして**スキャン場所**（Claude ディレクトリ / リポジトリルート）。`~/.claude` + `~/ghq` 以外の構成でも動きます。
...
- 📏 **サーバー真値のリミット表示（オプトイン）** — 設定のトグル（既定 OFF）を有効にすると、公式 `/usage` コマンドと*同じ* 5 時間・週次の使用率をリセットまでの時間・プランバッジ付きで表示します（プラン検出自体は完全ローカル）。Claude Code が保存済みの OAuth トークン（ファイルまたは Keychain）を再利用し、**そのトークンだけを Anthropic だけに**送ります。独立した第 2 のトグルで **Codex** も同様に有効化でき（`~/.codex/auth.json` のトークン → 宛先は OpenAI のみ）、Codex の 5 時間・週次の使用率とプランを表示します。この 2 つのオプトインが本アプリ唯一の通信機能で、それ以外は完全オフラインのままです。トークンの更新は行いません（失効時はそれぞれの CLI を開いてください）。手法は [CodexBar](https://github.com/steipete/CodexBar)（MIT）の調査に基づきます。
- 🤝 **プロバイダ比較** — OpenAI Codex CLI も使っている場合、Tools タブの「Providers」セクションでセッション数（シェアバー）と Codex のトークン使用量を Claude Code と並べて比較できます。`~/.codex/sessions/` のロールアウトログを直接読むだけで、OAuth も CLI 実行もしません。Codex が無いマシンではセクション自体が現れません。
- 🪞 **自己計測** — Tokfuel は*自分自身の* UI イベント（ポップオーバーを開いた、タブを切り替えた、設定を変えた。トランスクリプトの内容・プロジェクト名・コストは決して含みません）を `~/Library/Application Support/Tokfuel/events/` のローカル JSONL に記録し、今後の改善判断の根拠にします。Mac の外に出ないためデフォルト有効で、設定から表示・無効化・全削除ができます。
- 🔒 **100% ローカル** — データは外に出ません。
...
2. **Cost** — 期間合計（Today / 7d / 30d）・キャッシュヒット率・日次コストグラフ・モデル別コスト・retok の改善提案（タップで詳細）・高コストセッション。
3. **Tools** — 期間フィルタ（Today / 7d / 30d / All）、今日の利用量と前日比、Skills / MCP / Agents の日次積み上げグラフ、ランキング、ジャンル → リポジトリのドリルダウン。ヘッダー直下には今日のコスト・プロンプト数・セッション数の「Today」行がどのタブでも常時表示され、最後に選んだ期間は記憶されます。
