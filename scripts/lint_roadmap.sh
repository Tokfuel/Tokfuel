#!/bin/bash
# Lint the roadmaps/ tree: every CU item directory must hold a matching EN file and JA mirror,
# each with a CU-METADATA block and a valid Status (状態). Mirrors bajutsu's lint-roadmap idea,
...
shopt -s nullglob
dirs=(roadmaps/CU-*/)
[ ${#dirs[@]} -gt 0 ] || { err "no CU item directories found under roadmaps/"; exit 1; }

seen_ids=""
for dir in "${dirs[@]}"; do
  name="$(basename "$dir")"                       # CU-NNNN-<slug>
  id="${name%%-"${name#CU-????-}"}"               # CU-NNNN
  en="$dir$name.md"
  ja="$dir$name-ja.md"

  # Directory name shape
  [[ "$name" =~ ^CU-[0-9]{4}-[a-z0-9-]+$ ]] || err "$name: directory name must be CU-NNNN-<kebab-slug>"
  ...
  grep -q '<!-- CU-METADATA -->' "$en" || err "$name: EN file lacks <!-- CU-METADATA --> block"
  ...
  grep -q '<!-- CU-METADATA -->' "$ja" || err "$name: JA file lacks <!-- CU-METADATA --> block"
...
for readme in roadmaps/README.md roadmaps/README-ja.md; do
  ...
  done < <(grep -oE 'CU-[0-9]{4}-[a-z0-9-]+' "$readme" | sort -u)
done
